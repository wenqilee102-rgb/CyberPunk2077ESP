...
namespace GameUtils
{
    namespace EntityUtils
    {
        ...
        static void SetCachedHeadPosition(void* entity, const Utils::Vector3& pos)
        {
            if (!Utils::IsValidPtr(entity))
                return;
        ...
        Utils::Vector3 GetHeadPosition(void* entity)
        {
            ...
            auto targetingPos = GetHeadPositionFromTargeting(entity);
        ...
        Utils::Vector3 GetHeadPositionFromTargeting(void* entity)
        {
            ...
            {
                static bool loggedNoParts = false;
                if (!loggedNoParts)
                {
                    DBG_INFO("[Targeting] GetTargetParts returned no parts (result=%d, size=%d)", getPartsResult, partsArray.size);
                    loggedNoParts = true;
                }
                return Utils::Vector3(0, 0, 0);
            }

            static bool loggedParts = false;
            if (!loggedParts)
            {
                DBG_INFO("[Targeting] GetTargetParts returned %d parts", partsArray.size);
                loggedParts = true;
            }
            ...
            for (uint32_t i = 0; i < partsArray.size; i++)
            {
                ...
                {
                    static bool loggedPos = false;
                    if (!loggedPos)
                    {
                        DBG_INFO("[Targeting] Part[%d] position: (%.2f, %.2f, %.2f)", i, pos.X, pos.Y, pos.Z);
                        loggedPos = true;
                    }
                    SetCachedHeadPosition(entity, pos);
                    return pos;
                }
            }
