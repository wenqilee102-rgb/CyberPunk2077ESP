...
namespace GameUtils
{
    namespace CameraUtils
    {
        bool WorldToScreenWithDepth(const Utils::Vector3& worldPos, ImVec2& screenPos, float& depth, float screenWidth, float screenHeight)
        {
            ...
            if (!RttiUtils::g_ProjectPointFunc)
                return false;
            ...
            RttiUtils::g_ProjectPointFunc->Execute(&stack);
        ...
        bool IsInFieldOfView(const Utils::Vector3& worldPos, float screenWidth, float screenHeight)
        {
            ...
            if (screenResult.X < -1.0f || screenResult.X > 1.0f)
                return false;

            if (screenResult.Y < -1.0f || screenResult.Y > 1.0f)
                return false;
