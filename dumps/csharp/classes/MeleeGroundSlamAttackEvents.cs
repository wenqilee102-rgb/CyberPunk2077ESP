using WolvenKit.RED4.CR2W.Reflection;
using FastMember;
using static WolvenKit.RED4.CR2W.Types.Enums;

namespace WolvenKit.RED4.CR2W.Types
{
	[REDMeta]
	public class MeleeGroundSlamAttackEvents : MeleeAttackGenericEvents
	{
		private CHandle<gameStatModifierData_Deprecated> _knockdownImmunityModifier;
		private CHandle<gameStatModifierData_Deprecated> _stunImmunityModifier;

		[Ordinal(15)] 
		[RED("knockdownImmunityModifier")] 
		public CHandle<gameStatModifierData_Deprecated> KnockdownImmunityModifier
		{
			get => GetProperty(ref _knockdownImmunityModifier);
			set => SetProperty(ref _knockdownImmunityModifier, value);
		}

		[Ordinal(16)] 
		[RED("stunImmunityModifier")] 
		public CHandle<gameStatModifierData_Deprecated> StunImmunityModifier
		{
			get => GetProperty(ref _stunImmunityModifier);
			set => SetProperty(ref _stunImmunityModifier, value);
		}

		public MeleeGroundSlamAttackEvents(IRed4EngineFile cr2w, CVariable parent, string name) : base(cr2w, parent, name) { }
	}
}
