using WolvenKit.RED4.CR2W.Reflection;
using FastMember;
using static WolvenKit.RED4.CR2W.Types.Enums;

namespace WolvenKit.RED4.CR2W.Types
{
	[REDMeta]
	public class WeaponEvolutionHitPrereqCondition : BaseHitPrereqCondition
	{
		private CEnum<gamedataWeaponEvolution> _weaponEvolution;

		[Ordinal(3)] 
		[RED("weaponEvolution")] 
		public CEnum<gamedataWeaponEvolution> WeaponEvolution
		{
			get => GetProperty(ref _weaponEvolution);
			set => SetProperty(ref _weaponEvolution, value);
		}

		public WeaponEvolutionHitPrereqCondition(IRed4EngineFile cr2w, CVariable parent, string name) : base(cr2w, parent, name) { }
	}
}
